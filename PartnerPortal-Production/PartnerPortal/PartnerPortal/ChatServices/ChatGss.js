			...
			//alert(data[i].Key + data[i].Value);
				if (data[i].Key == 'EnableChatOnGSS') {
					if (!data[i].Value) {
						...
						window.open('https://starmicronics.desk.com:80/customer/widget/chats/new?', 'assistly_chat','resizable=1, status=0, toolbar=0,width=640,height=700');
					}
				}
            }
        },
