(function(h, g, c, j, e, k, f) { /*! Jssor */
    ...
    sliderWithThumb_init = function (divSliderWithThumbId) {
        var sliderWithThumbSlideshowTransitions = [
            { $Duration: 1500, x: -0.3, $During: { $Left: [0.3, 0.7] }, $Easing: { $Left: $Jease$.$InCubic, $Opacity: $Jease$.$Linear }, $Opacity: 2 }
        ];

        var sliderWithThumbOptions = {
            $AutoPlay: 1,
            $Idle: 3000,
            $SlideDuration: 1000,
        ...
        var sliderWithThumb = new $JssorSlider$(divSliderWithThumbId, sliderWithThumbOptions);

        /*responsive code begin*/
        /*remove responsive code if you don't want the slider scales while window resizing*/
        function sliderWithThumbSlider() {
            var refSize = sliderWithThumb.$Elmt.parentNode.clientWidth;
            if (refSize) {
                refSize = Math.min(refSize, 800);
                sliderWithThumb.$ScaleWidth(refSize);
            }
            else {
                window.setTimeout(sliderWithThumbSlider, 30);
            }
        }
        sliderWithThumbSlider();
        $Jssor$.$AddEvent(window, "load", sliderWithThumbSlider);
        $Jssor$.$AddEvent(window, "resize", sliderWithThumbSlider);
        $Jssor$.$AddEvent(window, "orientationchange", sliderWithThumbSlider);
        /*responsive code end*/

        $('.clickToStopPlay').on('click', function () {
            sliderWithThumb.$Pause();
        });
        $('.clickToStartPlay, .jssora02l, .jssora02r, .jssorb20').on('click', function () {
            sliderWithThumb.$Play();
        });
